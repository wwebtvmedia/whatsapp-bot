...
/**
 * Best-effort extraction of a document's title banner: the first meaningful
 * line of the extracted text (magazine covers put their masthead on top:
 * "TOUTES LES STRATÉGIES POUR RÉUSSIR N° 2319-2320 – 17 SEPTEMBRE 2026").
 * Repetition counts are a trap here — recurring captions ("CHIFFRES CLÉS")
 * outweigh a banner printed once. No meaningful first line → null.
 * @param {string} text
 * @returns {string|null}
 */
export function docMasthead(text) {
  for (const rawLine of String(text || '').split('\n')) {
    const line = rawLine.replace(/\s+/g, ' ').trim();
    if (line.length >= 12 && line.length <= 120) return line;
  }
  return null;
}

/**
 * Build the identity-card chunk indexed with every document: filename, title
 * banner, head of the text and the sommaire titles. Document-level questions
